...
public class CurrentMatrix {
    ...
    public CurrentMatrix(int height, int width, int duration, int transfromNum, int[][] matrix) {
        this.height = height;
        this.width = width;
    ...
    public void transform(){
        ...
        for (int y = 0; y < matrix.length; y++) {
            for (int x = 0; x < matrix[0].length; x++) {
                ...
                //等于3，则下一状态总是活
                if(matrix[y][x]==1){
                    if(nearNum<2||nearNum>3)nextMatrix[y][x]=0;
                    else nextMatrix[y][x]=1;
                }
                else{
                    if(nearNum==3)nextMatrix[y][x]=1;
                    else nextMatrix[y][x]=0;
                }
            }
        }
        matrix=nextMatrix;
